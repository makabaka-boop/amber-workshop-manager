...
app.post('/api/work-orders/:id/advance-stage', authenticateToken, (req, res) => {
  ...
  if (sandpaper_used && sandpaper_used > 0 && !sandpaper_id) {
    return res.status(400).json({ error: '填写砂纸用量时必须选择砂纸类型' });
  }
  
  if (polishing_paste_used && polishing_paste_used > 0 && !polishing_paste_id) {
    return res.status(400).json({ error: '填写抛光膏用量时必须选择抛光膏类型' });
  }
  ...
  if (polishing_paste_used && polishing_paste_used > 0 && polishing_paste_id) {
    db.get('SELECT stock_quantity FROM polishing_paste WHERE id = ?', [polishing_paste_id], (err, paste) => {
      if (err) return res.status(500).json({ error: err.message });
      if (!paste) return res.status(400).json({ error: '所选抛光膏不存在' });
      if (paste.stock_quantity < polishing_paste_used) {
        return res.status(400).json({ error: `抛光膏库存不足，当前库存: ${paste.stock_quantity}g` });
      }
      processAdvance();
    });
  } else {
...
app.get('/api/dashboard/stats', authenticateToken, (req, res) => {
  ...
  `, (err, stageData) => {
    ...
    db.get('SELECT COUNT(*) as total FROM work_orders', (err, totalResult) => {
      ...
      db.get("SELECT COUNT(*) as completed FROM work_orders WHERE status = 'completed'", (err, completedResult) => {
        if (err) return res.status(500).json({ error: err.message });
          ...
          `, (err, trendData) => {
            ...
            `, (err, reworkData) => {
              ...
              res.json({
                ...
                completedOrders: completedResult.completed,
